...
class MATHBenchmark(BaseBenchmark):
    ...
    @retry(stop=stop_after_attempt(5), wait=wait_fixed(1), retry=retry_if_exception_type((Exception, asyncio.TimeoutError)), reraise=True)
    async def _generate_output(self, graph, input_text):
        return await asyncio.wait_for(graph(input_text), timeout=REQUEST_TIMEOUT_SECONDS)
