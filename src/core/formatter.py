...
class XmlFormatter(BaseFormatter):
    ...
    def _get_field_description(self, field_name: str) -> str:
        ...
        if self.model and field_name in self.model.model_fields:
            ...
            return getattr(field_info, 'description', f'<your {field_name} here>')
        return f'<your {field_name} here>'
        ...
        try:
            ...
            for field_name in self._get_field_names():
                if field_name not in found_fields:
                    raise FormatError(f"Field '{field_name}' is missing.")
...
class CodeFormatter(BaseFormatter):
    ...
    def validate_response(self, response: str) -> Tuple[bool, Union[Dict[str, str], str, None]]:
        ...
        try:
            ...
            # If sanitize returned empty string, fall back to original code if it's syntactically valid
            if not sanitized_code.strip():
                # Check if the original code is syntactically valid Python
                try:
                    import ast
                    ast.parse(code)
                    # If it's valid Python, use it as is
                    sanitized_code = code
                except SyntaxError:
                    # If it's not valid Python, return failure
                    return False, None
...
class TextFormatter(BaseFormatter):
    ...
    def validate_response(self, response: str) -> Tuple[bool, Union[Dict[str, str], None]]:
        """
        For plain text formatter, we simply return the response wrapped in a dict
        """
        if response.strip():
